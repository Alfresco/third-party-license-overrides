...
LICENSE_PREFERENCE_ORDER = [
    ...
    # Not automatically approved for modified distributed.
    "CC0-1.0",
    "CDDL-1.0",
    "CDDL-1.1",
    "CPL-1.0",
...
parser = argparse.ArgumentParser(
    description="A script to generate CSV files containing third-party license information.",
...
parser.add_argument(
    ...
    "--combined",
    action="store_true",
    help="Create a single output file containing all libraries.",
)
parser.add_argument(
    "-d",
    "--desired",
    required=False,
    help="An ordered list of desired licenses delimited by |",
...
parser.add_argument(
    ...
    help="The output directory (will be created if necessary)",
)
parser.add_argument(
    "-p",
    "--project",
    required=True,
    help="The path to the project containing THIRD-PARTY.txt files",
...
parser.add_argument(
    ...
    "--version",
    required=True,
    help="The product version number being released",
    default=argparse.SUPPRESS,
)
...
for path, dirs, files in os.walk(root_dir):
    if "THIRD-PARTY.txt" in files and path.endswith(
...
if len(licenses_dirs) == 0:
    print(
        'Failed to find any generated files matching "/target/generated-sources/license/THIRD-PARTY.txt".'
    )
    exit(1)
...
if len(skipped_target_dirs) > 0:
    print(
        "No THIRD-PARTY.txt file found in the target directories of the following modules: {}".format(
            ", ".join(skipped_target_dirs)
        )
    )
...
for product in licenses_dirs:
    ...
    with open(licenses_dirs[product] + "/THIRD-PARTY.txt") as f:
        ...
        for line in re.split(r"[\n\r]", text):
            if (
                line.strip() == ""
                or re.match(r"Lists of [0-9]+ third-party dependencies.", line)
                or line == "The project has no dependencies."
            ):
                continue
...
def output_to_csv(project, version, license_information):
    ...
    output_file = "{}/{}-{}-3rd-Party-libraries.csv".format(
        output_dir, project, version
    )
    with open(output_file, "w", newline="") as csv_file:
        ...
        for jar in sorted(license_information.keys(), key=str.lower):
            ...
            product = (
                project
                if "product" not in license_information[jar]
                else license_information[jar]["product"]
            )
